...
import { Gamepad2, Sparkles, Trophy, Star, ChevronLeft, ChevronRight, MessageSquare, ExternalLink, ShieldCheck } from 'lucide-react';
import confetti from 'canvas-confetti';
import { sound } from '../utils/audio';

export default function Hero({ gameData }) {
  ...
  return (
    <section id="home" className="relative overflow-hidden pt-6 pb-12 px-4">
      {/* Background Stud Ambience */}
      <div className="absolute inset-0 bg-studs opacity-40 pointer-events-none" />
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[700px] h-[350px] bg-gradient-to-r from-roblox-cyan/15 via-roblox-pink/15 to-roblox-yellow/15 blur-3xl -z-10 rounded-full" />
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-[#18202f] border-4 border-black rounded-3xl p-6 md:p-10 shadow-stud-card relative overflow-hidden bg-studs">
          <div className="relative z-10 flex flex-col lg:flex-row items-center gap-8 lg:gap-10">
            
            {/* Game Icon */}
            <div className="relative flex-shrink-0 group">
              <div className="w-40 h-40 md:w-48 md:h-48 rounded-3xl bg-gradient-to-tr from-roblox-cyan via-roblox-yellow to-roblox-pink p-2 border-4 border-black shadow-stud-card group-hover:scale-105 transition-transform duration-300">
                <img 
                  src={gameData.logo} 
                  alt={gameData.title}
              ...
              <div className="absolute -bottom-2 -left-2 px-3 py-1 bg-black/90 rounded-xl border-2 border-roblox-cyan text-roblox-cyan font-game text-xs">
                Roblox Arcade
              </div>
            </div>
          ...
          {/* How to Play Box (Faithful to user's specification) */}
          <div className="mt-8 pt-8 border-t-3 border-slate-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {gameData.howToPlay.map((step, idx) => (
                <div
                  key={idx}
                  className="bg-slate-900/90 border-2 border-black rounded-2xl p-3.5 flex items-start gap-3 hover:border-slate-600 transition"
                >
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3 bg-slate-950/80 border-2 border-black rounded-2xl p-3 px-4 text-xs font-bubble text-slate-300">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-8 border-t-3 border-slate-800">
            {gameData.stats.map((stat, idx) => (
              <div 
                key={idx} 
                className="bg-slate-900/80 border-3 border-black rounded-2xl p-4 text-center shadow-md relative overflow-hidden group hover:border-slate-600 transition bg-studs"
              >
